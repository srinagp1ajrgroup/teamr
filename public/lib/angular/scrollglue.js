    ...
    function createDirective(module, attrName, direction){
        module.directive(attrName, ['$parse', '$window', '$timeout', function($parse, $window, $timeout){
            return {
                ...
                scope: { getgrouphistory: '&callbackFn',getchathistory: '&callbackFn', chatmessagestatus: '=',  topmiddleindex: '=' },
                link: function(scope, $el, attrs){
                    ...
                    $el.bind('scroll', function(){
                        if (Math.round(el.scrollTop) == 0) {
                            //for one to one chat
                            if (scope.chatmessagestatus == false) return;
